...
| SCR | P2 | Waarschuwingsscreenshots aanleveren (projectlead) en invoegen volgens 6C: Edge + Chrome downloadmelding, SmartScreen, macOS "kan niet worden geopend", Privacy en beveiliging, "beschadigd", Terminal | `ciosmentorendashboard/` (afbeeldingen + index.html) | Alt-tekst volgens 6C; axe wcag2aa lokaal | ⬜ |
...
## Wijziging uitvoering (projectlead, 2026-09-15)

- **Nieuwe pagina eerst live, daarna testen en delen:** T0 (koude installatie) gebeurt op de **nieuwe** pagina, na LP-SYS → D5. De inhoud van D1 wordt na T0 bijgesteld waar nodig.
- **Waarschuwingsscreenshots later:** eerste livegang zonder screenshots (tekst + geruststelling per stap); screenshots volgens 6C worden toegevoegd zodra de projectlead ze aanlevert (nieuwe taak SCR).
- **Publicatie:** bouwen op een branch in `ciosmentorendashboard`, lokale preview + axe + screenshots desktop/mobiel, **goedkeuring projectlead vóór merge naar `main`** (GitHub Pages publiceert direct vanaf `main`).
- **Contract met release-automatisering:** `update-landing-page.mjs` eist precies 3 release-downloadlinks. Herstelacties ("Probeer opnieuw", "Andere Mac-versie") zijn daarom knoppen zonder release-URL die de canonieke downloadknop van het paneel gebruiken (vastgelegd in T5-fixture `tests/fixtures/landing-doelopbouw.html`).
- **Lettertype:** Industry als OTF self-hosted (geen WOFF2-conversie beschikbaar; `fontTools` ontbreekt).
