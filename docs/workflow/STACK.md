# STACK.md — Volledig werkproces
# Superpowers · GStack · UI UX Pro Max · GSD · Claude Mem

> Verplaatst uit `CLAUDE.md` in v2.0.0. `CLAUDE.md` en dit document vormen samen de projectinstructies
> met de hoogste prioriteit; bij conflict tussen beide wint `CLAUDE.md`. Dit document staat niet elke
> sessie in context — `CLAUDE.md` zegt wanneer je welke sectie leest. Sectienummers zijn ongewijzigd,
> zodat bestaande verwijzingen (bv. "§7", "sectie 8") blijven kloppen. Wijzigingen volgen
> `docs/CLAUDE-MD-GOVERNANCE.md`; geen enkel framework schrijft naar dit document.

## 0. Sessieopstart — verplichte volgorde

De verplichte volgorde staat in **`CLAUDE.md` → Sessieopstart** (enige bron). Deze sectie bevat
alleen de aanvullende procedures: de stack-check (§0a) en de fallbacks hieronder.
...
Na installatie of bij een bestaand project: controleer of de aanwezige versies voldoen aan de minimumvereisten uit de footer van `CLAUDE.md` (`Superpowers v5+`, `GStack v1.26+`, `UI UX Pro Max = npm-latest`, `GSD v1.40+`, `Claude Mem v12+`).
...
2. **Na het inladen van meer dan 2 grote bestanden in één sessie** — groot = meer dan 200 regels of meer dan 5 KB; de verplichte sessiestart-bestanden (CLAUDE.md, STATE.md, DECISIONS.md, KNOWLEDGE.md) en de gelezen sectie van STACK.md tellen niet mee
...
CLAUDE.md                      Kern-instructies — hoogste prioriteit
docs/workflow/STACK.md         Volledig werkproces (dit document)
```
...
# Let op: /gsd-* schrijft naar .planning/ — neem uitkomst over in .gsd/ (zie CLAUDE.md, snelreferentie)
...
- Eigenaar, SemVer, git-workflow en changelog van CLAUDE.md en dit document: `docs/CLAUDE-MD-GOVERNANCE.md`.
  Lees dat document alleen wanneer CLAUDE.md of STACK.md wordt aangepast. Geen enkel framework schrijft naar CLAUDE.md of STACK.md.
